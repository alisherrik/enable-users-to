  ...
  const planResponse = await callGlm(apiKey, [
    {
      ...
      content: [
        ...
        "- If the issue says to put styles INLINE in <style> tags, plan accordingly — do NOT plan to write styles.css.",
        "- If the issue title says 'integration', plan to extract inline styles/scripts from all pages into shared files.",
        "- Do NOT plan to generate any images. Use inline SVG or Unicode only.",
        "- For each HTML page: plan specific sections with detailed visual descriptions (hero gradient, glass cards, animated grids, etc.).",
        "- For styles.css (integration only): plan to merge all inline <style> blocks into one unified file.",
        "- For script.js (integration only): plan to merge all inline <script> blocks into one unified file.",
    ...
    const fileResponse = await callGlm(apiKey, [
      {
        ...
        content: [
          ...
          "You MUST use design tokens for colors and spacing.",
          ...
          "Do NOT generate or reference any images. Use inline SVG icons or Unicode symbols only. No <img> tags, no picsum.photos.",
          "",
          "PAGE AGENT MODE (when building a single HTML page):",
          "- Put ALL CSS inside a <style> tag in the <head>. Do NOT create or modify styles.css.",
          "- Put ALL JavaScript inside a <script> tag before </body>. Do NOT create or modify script.js.",
          "- Keep the existing <link rel='stylesheet' href='./styles.css'> and <script src='./script.js'></script> references.",
          "- The page must be FULLY FUNCTIONAL with its inline styles and scripts.",
          "",
          "INTEGRATION AGENT MODE (when issue title says 'integration'):",
          "- For styles.css: read ALL HTML pages, extract every <style> block, merge into unified styles.css.",
          "- For script.js: read ALL HTML pages, extract every <script> block (not src ones), merge into unified script.js.",
          "- For HTML pages: remove inline <style> and <script> blocks, keep only external references.",
          "- Ensure nav consistency, fix cross-page links, unify active states.",
          "",
          ...
          "- Put ALL CSS inside a <style> tag in <head>. Put ALL JS inside a <script> tag before </body>.",
          ...
          "- Do NOT use any <img> tags or image URLs. Use inline SVG icons or Unicode only.",
